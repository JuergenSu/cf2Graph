...
for line in $names; do
  ...
  echo "  label=\"ORG:$name\""
  echo "  style=filled; color=lightgrey"
    ...
    for spaceline in $spaces; do
        ...
        echo "  style=filled; color=white"
          ...
          cf curl /v2/spaces/$spaceguid/service_instances | jq -r '.resources[] | "\"" + .metadata.guid + "\"" + " [label=\"Service:" + .entity.name + "\" shape=box]"'
